...
const MAX_TOTAL_ATTEMPTS = 50;
                                                                                 async function fetchReels(inputFile) {                                             try {                                                                              // Read the file containing reels URLs                                           let fileContent;                                                                 try {                                                                              fileContent = await fs.readFile(inputFile, "utf-8");                           } catch (readErr) {                                                                console.error(`Failed to read ${inputFile}:`, readErr.message);                  return; // Cannot proceed without URLs
    }                                                                                                                                                                 let urls = Array.from(                                                             new Set(                                                                           fileContent                                                                        .split("\n")
          .map((line) => line.trim())
          .filter(Boolean)
      )
    );
